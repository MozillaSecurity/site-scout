...
class VisitSummary:
    ...
    Attributes:
        ...
        alias: Location visited (URL or other handle).
    ...
    def _process_complete(self, log_path: Path) -> int:
        ...
        while self._complete:
            ...
            if summary.state == State.LOAD_FAILURE:
                LOG.info("Page load failure: '%s'", summary.alias)
            elif summary.state == State.NOT_FOUND:
                LOG.info("Server Not Found: '%s'", summary.alias)
        ...
        for idx in reversed(range(len(self._urls))):
            if (
                ...
                self._summaries.append(
                    VisitSummary(0, url.alias, False, False, state=state)
                )
                removed += 1
        if removed > 0:
        ...
        while self._urls or self._active:
            ...
            if self._urls and (len(self._active) < instance_limit):
                ...
                if (
                    ...
                    LOG.debug("domain rate limit hit (%s)", next_url.alias)
                ...
                # attempt to launch browser and visit url
                    ...
                    short_url = trim(next_url.alias, 80)
                    LOG.info(
                        "[%02d/%02d] '%s'",
